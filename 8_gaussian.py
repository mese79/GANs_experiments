...
from data import (sample_noise, sample_8_gaussians)
from models import (Discriminator8G, Generator8G)
from losses import (standard_d_loss, standard_g_loss, heuristic_g_loss)
from utility import (clear_line, clear_collection)
...
def main(argv=[]):
    ...
    z_dim: int = 2
    data_dim: int = 2
    d_hidden_size: int = 120
    g_hidden_size: int = 120
    ...
    experiment_dir = Path(f'./experiments/8_gaussian_exp_{time():.0f}')
    if save_model:
    ...
    # plots
    _data = sample_8_gaussians(3000, torch.device('cpu')).numpy()
    fig, ax_data, ax_loss, ax_disc = prepare_plots(_data, experiment_info, '8 Gaussians')
    ...
    # Creating models
    D = Discriminator8G(data_dim, d_hidden_size).to(device)
    G = Generator8G(z_dim, g_hidden_size, data_dim).to(device)
    print(f'\n\n{D}\n\n{G}\n\n')
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for k in range(1):
            real_data = sample_8_gaussians(minibatch_size, device)
...
def prepare_plots(data, info, title=''):
    fig: plt.Figure = plt.figure(1, figsize=(15, 6.4))
    fig.canvas.set_window_title(title)

    ax_data = fig.add_subplot(2, 3, (1, 4))
    ax_loss = fig.add_subplot(2, 3, (2, 3))
    ax_disc = fig.add_subplot(2, 3, (5, 6))
    ...
    ax_data.set_xlim([-4.0, 4.0])
    ax_data.set_ylim([-5.2, 5.2])
    # ax_data.axis('equal')
    ax_data.annotate(
        info.replace('\n', '  '),
        xy=(0, 0),
