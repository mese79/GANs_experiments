...
from data import (sample_noise, sample_25_gaussians)
from models import (Discriminator25G, Generator25G)
...
def main(argv=[]):
    ...
    # plots
    _data = sample_25_gaussians(3000, torch.device('cpu')).numpy()
    fig, ax_data, ax_loss, ax_disc = prepare_plots(_data, experiment_info, '25 Gaussians')
    ...
    # Creating models
    D = Discriminator25G(data_dim, d_hidden_size).to(device)
    G = Generator25G(z_dim, g_hidden_size, data_dim).to(device)
    print(f'\n\n{D}\n\n{G}\n\n')
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for k in range(1):
            real_data = sample_25_gaussians(minibatch_size, device)
